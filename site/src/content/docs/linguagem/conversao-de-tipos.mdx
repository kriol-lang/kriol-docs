...
// Opção C: método no valor
nter n = 3.7.para_nter();
```
...
O alargamento `nter` → `num` está implementado. A conversão explícita (truncagem, `textu` ↔ números) ainda está em desenvolvimento e a sintaxe definitiva não está finalizada.
